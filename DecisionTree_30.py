# -*- coding: utf-8 -*-
"""
 @Time    : 2018/12/12 0012 上午 9:19
 @Author  : Shanshan Wang
 @Version : Python3.5
 @Function: 使用决策树进行鸢尾花分类的实例
"""
import numpy as np
import pandas as pd
...
from sklearn.feature_selection import chi2 #卡方统计值
...
#因为需要体现的是分类模型，DecisionTreeClassifier是分类算法，要求y必须是int型
...
print('原始数据各个特征属性的缩放数据集：',ss.scale_)
...
print('*'*58)
print(x2.shape) #(100, 100)
# flat 扁平化操作
x_show = np.dstack((x1.flat, x2.flat))[0] #测试点

print(x_show.shape) #10000, 2)

y_show_hat = model.predict(x_show) #预测值

y_show_hat = y_show_hat.reshape(x1.shape)  #使之与输入的形状相同
print(y_show_hat.shape) #(100, 100)
print(y_show_hat[0])
